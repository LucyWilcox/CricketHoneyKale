...
class Danger_Factor(object):
	...
	def level_two(self, food_type):
		...
		current_salad_2 = []
		current_smoothie_2 = []
		instruction1 = make_recipe(food_type).replace('!', '').replace(',', '').replace(':', '').split()
		instruction2 = make_recipe(food_type).replace('!', '').replace(',', '').replace(':', '').split()
		for word in instruction1:
			if word in smoothie_ingredients and food_type == 'smoothie':
				current_smoothie_1.append(word)
			if word in salad_ingredients and food_type == 'salad':
	...
	def level_three(self, food_type):
		...
		second_recipe = make_recipe(food_type)
...
if __name__ == '__main__':
	...
	print Bizarre_recipe.level_one('smoothie')